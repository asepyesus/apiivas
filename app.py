...
def numbers_raw_debug():
    """
    Debug: Coba berbagai cara fetch data test numbers dari iVAS.
    Lihat response mentah untuk tau format yang benar.
    """
    sessions = login_all_accounts()
    if not sessions:
    ...
    try:
        r1, _ = do_request(account, "GET",
            f"{BASE_URL}/portal/numbers/test",
            headers={"Referer": f"{BASE_URL}/portal/numbers/test",
                     "Accept": "application/json, text/javascript, */*; q=0.01",
                     "X-Requested-With": "XMLHttpRequest"})
        h1 = decode_response(r1) if r1 else ""
        results["test1_no_params"] = {
            "status": r1.status_code if r1 else None,
            "is_json": r1.headers.get("content-type","") if r1 else "",
            "preview": h1[:800]
        }
    except Exception as e:
        results["test1_no_params"] = {"error": str(e)}

    # Test 2: GET dengan DataTables params standar
    try:
        r2, _ = do_request(account, "GET",
            f"{BASE_URL}/portal/numbers/test",
            headers={"Referer": f"{BASE_URL}/portal/numbers/test",
                     "Accept": "application/json, text/javascript, */*; q=0.01",
                     "X-Requested-With": "XMLHttpRequest"})
        # Rebuild dengan params di URL
        qs = "draw=1&columns[0][data]=range&columns[0][name]=terminations.range&columns[1][data]=test_number&columns[1][name]=terminations.test_number&order[0][column]=0&order[0][dir]=asc&start=0&length=50&search[value]=&search[regex]=false"
        r2b, csrf2 = do_request(account, "GET",
            f"{BASE_URL}/portal/numbers/test?{qs}",
            headers={"Referer": f"{BASE_URL}/portal/numbers/test",
                     "Accept": "application/json, text/javascript, */*; q=0.01",
                     "X-Requested-With": "XMLHttpRequest"})
        h2 = decode_response(r2b) if r2b else ""
        results["test2_datatable_params"] = {
            "status": r2b.status_code if r2b else None,
            "content_type": r2b.headers.get("content-type","") if r2b else "",
            "preview": h2[:800]
        }
    except Exception as e:
        results["test2_datatable_params"] = {"error": str(e)}

    # Test 3: POST dengan DataTables params
    try:
        r3, _ = do_request(account, "POST",
            f"{BASE_URL}/portal/numbers/test",
            data={
                "draw": "1",
                "start": "0",
            ...
            headers={"Referer": f"{BASE_URL}/portal/numbers/test",
                     "Accept": "application/json, text/javascript, */*; q=0.01",
                     "X-Requested-With": "XMLHttpRequest"})
        h3 = decode_response(r3) if r3 else ""
        results["test3_post_datatable"] = {
            "status": r3.status_code if r3 else None,
            "content_type": r3.headers.get("content-type","") if r3 else "",
            "preview": h3[:800]
        }
    except Exception as e:
        results["test3_post_datatable"] = {"error": str(e)}

    # Test 4: GET /portal/numbers/test/data atau /test/list
    for suffix in ["/data", "/list", "/get", "/json"]:
        try:
            r4, _ = do_request(account, "GET",
                f"{BASE_URL}/portal/numbers/test{suffix}",
                headers={"Referer": f"{BASE_URL}/portal/numbers/test",
                         "Accept": "application/json, */*; q=0.01",
                         "X-Requested-With": "XMLHttpRequest"})
            h4 = decode_response(r4) if r4 else ""
            results[f"test4_{suffix}"] = {
                "status": r4.status_code if r4 else None,
                "preview": h4[:400]
            }
        except Exception as e:
            results[f"test4_{suffix}"] = {"error": str(e)}
