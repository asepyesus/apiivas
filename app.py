...
def get_numbers(account, range_name, from_date, to_date):
...
# ════════════════════════════════════════════════════════
# SESSION CACHE — auto re-login kalau session expired
# ════════════════════════════════════════════════════════
...
def _is_session_expired(response):
    ...
    try:
        ...
        if any(k in snippet for k in ('forgot your password', 'login to your account', '<form.*action.*login')):
            return True
    except Exception:
        pass
    return False
...
def do_request(account, method, url, data=None, headers=None):
    ...
    otomatis, lalu retry. Tidak ada timeout.
    """
    data = dict(data) if data else {}
    for attempt in range(3):
        session = get_session(account, force=(attempt > 0))
        if not session or not session.get("ok"):
            logger.error(f"[REQ] Login gagal {account['email']} attempt {attempt+1}")
            if attempt < 2:
                time.sleep(2)
            continue
        ...
        try:
            ...
            if _is_session_expired(resp):
                logger.warning(f"[REQ] Expired {account['email']} attempt {attempt+1}, re-login...")
                with _session_lock:
                    if email in _session_cache:
                        _session_cache[account['email']]["ok"] = False
                continue

            return resp, csrf

        except Exception as e:
            logger.error(f"[REQ] Error {account['email']} attempt {attempt+1}: {e}")
            if attempt < 2:
                time.sleep(2)
...
def fetch_received_from_session(session, from_date, to_date):
    ...
    for rng in ranges:
        rng_name = rng["name"]
        numbers  = get_numbers(account, rng_name, from_date, to_date)
        for num in numbers:
...
def debug_ranges_raw():
    ...
    session = sessions[0]
    try:
        acc_d = next((a for a in ACCOUNTS if a["email"] == session["email"]), None)
        resp, _ = do_request(
            acc_d, "POST",
            f"{BASE_URL}/portal/sms/received/getsms",
            data={"from": to_ivas_date(date_str), "to": to_ivas_date(date_str)},
            headers=ajax_hdrs(),
        )
        html           = decode_response(resp) if resp else ""
        parsed_ranges  = get_ranges(acc_d, date_str, date_str)
...
def debug_numbers():
    ...
    session = sessions[0]
    try:
        acc_d2 = next((a for a in ACCOUNTS if a["email"] == session["email"]), None)
        resp, _ = do_request(
            acc_d2, "POST",
            f"{BASE_URL}/portal/sms/received/getsms/number",
            data={"start": to_ivas_date(date_str), "end": to_ivas_date(date_str), "range": range_name},
            headers=ajax_hdrs(),
        )
        html    = decode_response(resp) if resp else ""
        numbers = get_numbers(acc_d2, range_name, date_str, date_str)
...
def debug_sms():
    ...
    session = sessions[0]
    try:
        acc_d3 = next((a for a in ACCOUNTS if a["email"] == session["email"]), None)
        resp, _ = do_request(
            acc_d3, "POST",
            f"{BASE_URL}/portal/sms/received/getsms/number/sms",
            data={"start": to_ivas_date(date_str), "end": to_ivas_date(date_str),
                  "Number": number, "Range": range_name},
            headers=ajax_hdrs(),
        )
        html = decode_response(resp) if resp else ""
        msg  = get_sms(acc_d3, number, range_name, date_str, date_str)
...
def list_my_numbers():
    """
    GET /numbers
    Ambil semua nomor di My Numbers (/portal/numbers).
    Response: list of {number, range_name, rates, limit_by_range, sid_range_limit, sid_did_limit, number_id}
    """
    sessions = login_all_accounts()
    if not sessions:
        return jsonify({"error": "Semua akun gagal login"}), 500
    ...
    for session in sessions:
        ...
        if not account:
            continue
        try:
            resp, _ = do_request(account, "GET", f"{BASE_URL}/portal/numbers",
                                 headers={"Referer": f"{BASE_URL}/portal/numbers"})
            if resp is None or resp.status_code != 200:
                errors.append({"account": email, "error": f"HTTP {resp.status_code if resp else 'None'}"})
                continue

            html = decode_response(resp)
            soup = BeautifulSoup(html, "html.parser")

            # Cari tabel dengan kolom Number | Range Name
            for tbl in soup.find_all("table"):
                ths = [th.get_text(strip=True).lower() for th in tbl.find_all("th")]
                if not any("number" in h for h in ths):
                    continue

                # Cari index kolom
                num_idx   = next((i for i, h in enumerate(ths) if h == "number"), 0)
                range_idx = next((i for i, h in enumerate(ths) if "range" in h), 1)
                ...
                for row in tbl.find_all("tr"):
                    ...
                    # Ambil number_id dari tombol hapus (onclick / data-id / href)
                    number_id = None
                    action_td = tds[-1]  # kolom Action biasanya paling akhir
                    btn = action_td.find("button") or action_td.find("a")
                    if btn:
                        for attr in ("data-id", "data-number", "value"):
                            if btn.get(attr):
                                number_id = btn.get(attr)
                                break
                        if not number_id:
                            oc = btn.get("onclick", "")
                            m  = re.search(r"['\"]?(\d{5,15})['\"]?", oc)
                            if m:
                                number_id = m.group(1)
                        if not number_id:
                            href = btn.get("href", "")
                    ...
                    num_text   = re.sub(r"\D", "", tds[num_idx].get_text(strip=True))
                    range_text = tds[range_idx].get_text(strip=True)
                    ...
                    entry = {
                        "account":         email,
                        "number":          num_text,
                        "range_name":      range_text,
                        "number_id":       number_id,
                    }
                    # Kolom tambahan kalau ada
                    if len(tds) > 2:
                        entry["rates"]           = tds[2].get_text(strip=True) if len(tds) > 2 else ""
                        entry["limit_by_range"]  = tds[3].get_text(strip=True) if len(tds) > 3 else ""
                        entry["sid_range_limit"] = tds[4].get_text(strip=True) if len(tds) > 4 else ""
                        entry["sid_did_limit"]   = tds[5].get_text(strip=True) if len(tds) > 5 else ""
                    all_numbers.append(entry)
                break  # hanya tabel pertama yang match

        except Exception as e:
            errors.append({"account": email, "error": str(e)})
            logger.error(f"[NUMBERS-LIST] Error {email}: {e}")
...
@app.route("/numbers/add", methods=["POST", "GET"])
def add_number():
    """
    POST /numbers/add
    Tambah nomor ke range di /portal/numbers/test.

    Body JSON atau query params:
      - range_name : nama range, contoh "PERU 543"
      - number     : nomor telepon, contoh "51910550499"
      - account    : (opsional) email akun, default akun pertama

    Flow iVAS:
      1. GET /portal/numbers/test  → dapat csrf + daftar range
      2. Klik tombol + di baris range yang sesuai → dapat range_id
      3. POST addnumber endpoint dengan number + range_id
    """
    if request.method == "GET":
        ...
        range_name = data.get("range_name", "") or request.form.get("range_name", "")
        number     = data.get("number", "")     or request.form.get("number", "")
        account_email = data.get("account", "") or request.form.get("account", "")

    if not range_name or not number:
        return jsonify({
            "error":  "Parameter range_name dan number wajib",
            "contoh": "/numbers/add?range_name=PERU 543&number=51910550499"
        }), 400
    ...
    # Pilih akun
    if account_email:
        target_sessions = [s for s in sessions if s["email"] == account_email]
    else:
        target_sessions = sessions[:1]  # default akun pertama

    if not target_sessions:
        return jsonify({"error": f"Akun {account_email} tidak ditemukan atau gagal login"}), 404
    ...
    try:
        # Step 1: GET /portal/numbers/test → dapat csrf + range_id
        resp_page, csrf = do_request(account, "GET",
                                     f"{BASE_URL}/portal/numbers/test",
                                     headers={"Referer": f"{BASE_URL}/portal/numbers/test"})
        if resp_page is None:
            return jsonify({"error": "Gagal akses halaman Test Numbers"}), 500

        html_page = decode_response(resp_page)
        soup_page = BeautifulSoup(html_page, "html.parser")

        # Cari range_id dari tabel: baris yang Range Name == range_name → tombol + → onclick/data
        range_id  = None
        add_url   = f"{BASE_URL}/portal/numbers/test/store"  # default URL

        # Cari di tabel
        for tbl in soup_page.find_all("table"):
            for row in tbl.find_all("tr"):
                tds = row.find_all("td")
                if not tds:
                    continue
                row_range = tds[0].get_text(strip=True)
                if row_range.lower() != range_name.lower():
                    continue
                # Ketemu baris yang sesuai → cari tombol + / action
                btn = row.find("button") or row.find("a", class_=True)
                if btn:
                    for attr in ("data-id", "data-range", "value", "data-range-id"):
                        if btn.get(attr):
                            range_id = btn.get(attr)
                            break
                    if not range_id:
                        oc = btn.get("onclick", "")
                        m  = re.search(r"['\"](\w+)['\"]", oc)
                    ...
                    href = btn.get("href", "")
                    if href and href != "#":
                        add_url = BASE_URL + href if href.startswith("/") else href
                break
            if range_id:
                break

        # Fallback: range_id = range_name dengan underscore
        if not range_id:
            range_id = range_name.replace(" ", "_")
            logger.warning(f"[ADD-NUMBER] range_id tidak ditemukan, pakai fallback: {range_id}")

        # Step 2: POST untuk tambah nomor
        # iVAS biasanya: POST /portal/numbers/test/store dengan {number, range_id, _token}
        post_data = {
            "number":   number,
            "range_id": range_id,
            "range":    range_id,
        ...
        resp_add, _ = do_request(
            account, "POST", add_url,
            data=post_data,
            headers=ajax_hdrs(referer=f"{BASE_URL}/portal/numbers/test"),
        )

        if resp_add is None:
            return jsonify({"error": "Gagal POST ke iVAS"}), 500

        html_add  = decode_response(resp_add)
        success   = resp_add.status_code in (200, 201, 302)

        # Deteksi pesan sukses/gagal dari response
        msg_found = ""
        try:
            json_resp = resp_add.json()
            msg_found = json_resp.get("message", json_resp.get("msg", str(json_resp)))
            if json_resp.get("status") in ("success", "ok", True, 1):
                success = True
            elif json_resp.get("status") in ("error", "fail", False, 0):
                success = False
        except Exception:
            # Bukan JSON, cek HTML
            if any(k in html_add.lower() for k in ("success", "berhasil", "added")):
                success = True
                msg_found = "Number added successfully"
            elif any(k in html_add.lower() for k in ("error", "gagal", "failed", "already")):
                success = False
                msg_found = "Failed to add number"
        ...
        return jsonify({
            ...
            "range_id":    range_id,
            "number":      number,
            "http_status": resp_add.status_code,
            "message":     msg_found,
            "add_url":     add_url,
        })
...
@app.route("/numbers/delete", methods=["POST", "GET"])
def delete_number():
    """
    POST /numbers/delete
    Hapus/return nomor ke sistem dari My Numbers (/portal/numbers).

    Body JSON atau query params:
      - number  : nomor telepon yang mau dihapus, contoh "51910550499"
      - account : (opsional) email akun

    Flow iVAS:
      1. GET /portal/numbers → dapat list nomor + number_id tiap baris
      2. POST /portal/numbers/return/{number_id} atau AJAX return endpoint
    """
    if request.method == "GET":
        number        = request.args.get("number", "")
        account_email = request.args.get("account", "")
        ...
        number        = data.get("number", "")     or request.form.get("number", "")
        account_email = data.get("account", "")    or request.form.get("account", "")
        number_id     = data.get("number_id", "")  or request.form.get("number_id", "")

    if not number:
        return jsonify({
            "error":  "Parameter number wajib",
            "contoh": "/numbers/delete?number=51910550499"
        }), 400
    ...
    for session in target_sessions:
        ...
        try:
            # Step 1: GET /portal/numbers → cari number_id kalau belum dikasih
            found_id    = number_id
            found_range = ""

            if not found_id:
                resp_list, _ = do_request(account, "GET", f"{BASE_URL}/portal/numbers",
                                          headers={"Referer": f"{BASE_URL}/portal/numbers"})
                if resp_list is None:
                    continue

                html_list = decode_response(resp_list)
                soup_list = BeautifulSoup(html_list, "html.parser")

                for tbl in soup_list.find_all("table"):
                    ths = [th.get_text(strip=True).lower() for th in tbl.find_all("th")]
                    if not any("number" in h for h in ths):
                        continue
                    ...
                    for row in tbl.find_all("tr"):
                        ...
                        if len(tds) < 2:
                            continue
                        row_num = re.sub(r"\D", "", tds[num_idx].get_text(strip=True))
                        if row_num != re.sub(r"\D", "", number):
                            continue

                        found_range = tds[range_idx].get_text(strip=True)

                        # Cari ID dari tombol hapus
                        action_td = tds[-1]
                        btn = action_td.find("button") or action_td.find("a")
                        if btn:
                            for attr in ("data-id", "data-number", "value", "data-number-id"):
                                if btn.get(attr):
                                    found_id = btn.get(attr)
                                    break
                            if not found_id:
                                oc = btn.get("onclick", "")
                                m  = re.search(r"return\w*\s*\(\s*['\"]?(\d+)['\"]?", oc, re.I)
                                if m:
                                    found_id = m.group(1)
                                if not found_id:
                                    m = re.search(r"['\"]?(\d+)['\"]?", oc)
            ...
            if not found_id:
                logger.warning(f"[DELETE-NUMBER] number_id tidak ditemukan untuk {number} di {email}")
                continue

            # Step 2: POST return
            # iVAS: POST /portal/numbers/return/{id} atau /portal/numbers/{id}/return
            for return_url in [
                f"{BASE_URL}/portal/numbers/return/{found_id}",
                f"{BASE_URL}/portal/numbers/{found_id}/return",
                f"{BASE_URL}/portal/numbers/{found_id}",
            ]:
                resp_del, _ = do_request(
                    account, "POST", return_url,
                    data={"number": number, "id": found_id},
                    headers=ajax_hdrs(referer=f"{BASE_URL}/portal/numbers"),
                )
                if resp_del is None:
                    continue

                html_del = decode_response(resp_del)
                success  = resp_del.status_code in (200, 201, 302)
                msg_del  = ""

                try:
                    jr = resp_del.json()
                    msg_del = jr.get("message", jr.get("msg", str(jr)))
                    if jr.get("status") in ("success", "ok", True, 1):
                        success = True
                    elif jr.get("status") in ("error", "fail", False, 0):
                        success = False
                except Exception:
                    if any(k in html_del.lower() for k in ("success", "returned", "berhasil")):
                        success = True
                        msg_del = "Number returned successfully"

                if success or resp_del.status_code == 200:
                    return jsonify({
                        "status":      "ok",
                        "success":     True,
                        "account":     email,
                        "number":      number,
                        "number_id":   found_id,
                        "range_name":  found_range,
                        "http_status": resp_del.status_code,
                        "message":     msg_del or "Number deleted/returned",
                        "return_url":  return_url,
                    })

        except Exception as e:
            logger.error(f"[DELETE-NUMBER] Error {email}: {e}")

    return jsonify({
        "status":  "error",
        "success": False,
        "number":  number,
        "error":   "Gagal hapus nomor — number_id tidak ditemukan atau semua akun gagal",
    }), 500


@app.route("/numbers/debug-page")
def debug_numbers_page():
    """
    Debug: lihat raw HTML dari /portal/numbers dan /portal/numbers/test
    Berguna untuk inspect struktur tabel + cara dapat number_id.
    """
    page    = request.args.get("page", "my")  # "my" = /portal/numbers, "test" = /portal/numbers/test
    account_email = request.args.get("account", "")
    ...
    if account_email:
        target = next((s for s in sessions if s["email"] == account_email), sessions[0])
    else:
        target = sessions[0]
    ...
    url = f"{BASE_URL}/portal/numbers/test" if page == "test" else f"{BASE_URL}/portal/numbers"

    try:
        resp, _ = do_request(account, "GET", url, headers={"Referer": url})
        if resp is None:
            return jsonify({"error": "Gagal akses halaman"}), 500

        html = decode_response(resp)
        soup = BeautifulSoup(html, "html.parser")

        # Kumpulkan info berguna
        tables_info = []
        for i, tbl in enumerate(soup.find_all("table")):
            ...
            for row in tbl.find_all("tr")[:5]:
                ...
                if tds:
                    rows.append({
                        "cells": [td.get_text(strip=True)[:80] for td in tds],
                        "buttons": [{"tag": str(btn)[:300]} for btn in row.find_all(["button", "a"])],
                    })
            tables_info.append({
                "index": i,
                "headers": ths,
                "id":      tbl.get("id", ""),
                "data_url": tbl.get("data-url", ""),
                "sample_rows": rows
            })

        # Cari semua onclick yang ada angka
        all_onclicks = [
            el.get("onclick", "")
            for el in soup.find_all(onclick=True)
            if re.search(r"\d{3,}", el.get("onclick", ""))
        ][:30]

        # Extract DataTables ajax URL dari inline script
        ajax_urls  = re.findall(r"""ajax\s*[=:]\s*['"]([^'"]+)['"]""", html)
        ajax_urls += re.findall(r"""url\s*:\s*['"]([^'"]+portal[^'"]+)['"]""", html)
        ajax_urls  = list(dict.fromkeys(ajax_urls))  # deduplicate

        # Extract semua form action
        form_actions = [f.get("action", "") for f in soup.find_all("form") if f.get("action")]

        # Cari JS yang menyebut 'numbers' atau 'store' atau 'return'
        script_snippets = []
        for script in soup.find_all("script"):
            txt = script.get_text()
            if any(k in txt for k in ("numbers", "store", "return", "addNumber", "deleteNumber", "DataTable")):
                snippet = txt.strip()[:1500]
                if snippet:
                    script_snippets.append(snippet)
        ...
        return jsonify({
            ...
            "all_onclicks":    all_onclicks,
            "script_snippets": script_snippets[:5],
            "html_preview":    html[:3000],
        })
