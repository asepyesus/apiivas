...
def numbers_test_list():
    ...
    List semua Test Numbers dari /portal/numbers/test.
    Params: search, limit (default 100), account
    """
    sessions = login_all_accounts()
    if not sessions:
    ...
    target  = [s for s in sessions if s["email"] == acc_email] if acc_email else sessions

    all_numbers, errors, total_avail = [], [], 0
    for session in target:
        ...
        try:
            ...
            total_avail = max(total_avail, total)
            for row in rows:
                all_numbers.append({
                    "account":           email,
            ...
            logger.info(f"[TEST-LIST] {email}: {len(rows)}/{total}")
        except Exception as e:
...
def numbers_my_list():
    """
    GET /numbers/my-list
    List My Numbers dari /portal/numbers.
    Params: search, limit (default 100), account
    """
    sessions = login_all_accounts()
    if not sessions:
    ...
    return jsonify({"status":"ok","total":len(all_numbers),"numbers":all_numbers,"errors":errors})
...
def add_number():
    ...
    Params: range_name, number, account (opsional)
    Contoh: /numbers/add?range_name=PERU 543&number=51910550499
    iVAS: POST /portal/numbers/termination/number/add
    """
    if request.method == "GET":
        ...
        acc_email  = d.get("account","")    or request.form.get("account","")

    if not range_name or not number:
        return jsonify({"error":"Parameter range_name dan number wajib",
                        "contoh":"/numbers/add?range_name=PERU 543&number=51910550499"}), 400

    sessions = login_all_accounts()
    if not sessions:
        return jsonify({"error":"Login gagal"}), 500

    target  = [s for s in sessions if s["email"] == acc_email] if acc_email else sessions[:1]
    if not target:
...
def delete_number():
    ...
    Params: number_id ATAU number, account (opsional)
    Contoh: /numbers/delete?number_id=788486
            /numbers/delete?number=93766304206
    iVAS: POST /portal/numbers/termination/details  {id: number_id}
    """
    if request.method == "GET":
    ...
    if not number_id and not number:
        return jsonify({"error":"Parameter number_id atau number wajib",
                        "contoh_1":"/numbers/delete?number_id=788486",
                        "contoh_2":"/numbers/delete?number=93766304206"}), 400
    ...
    # Auto-search number_id kalau hanya punya nomor
    if not number_id and number:
        for session in target:
            ...
            for row in rows:
                clean = re.sub(r"\D","", str(row.get("test_number","")))
                if clean == re.sub(r"\D","", number):
                    number_id   = _get_number_id(row)
    ...
    if not number_id:
        return jsonify({"status":"error",
                        "error":f"number_id tidak ditemukan untuk number={number}. Cek /numbers/test-list"}), 404

    if acc_email:
        target = [s for s in sessions if s["email"] == acc_email] or target
...
def numbers_raw_debug():
    """Debug: coba berbagai format request ke iVAS test numbers."""
    sessions = login_all_accounts()
    if not sessions:
        return jsonify({"error":"Login gagal"}), 500
    ...
    try:
        ...
        results["test1_plain_get"] = {"status": r.status_code if r else None, "preview": h[:500]}
    except Exception as e:
        results["test1_plain_get"] = {"error": str(e)}

    # Test 2: GET dengan DataTables params + XHR header (ini yang confirmed work)
    try:
        qs = "draw=1&columns[0][data]=range&columns[0][name]=terminations.range&columns[1][data]=test_number&columns[1][name]=terminations.test_number&order[0][column]=0&order[0][dir]=asc&start=0&length=5&search[value]=&search[regex]=false"
        r, _ = do_request(account, "GET", f"{BASE_URL}/portal/numbers/test?{qs}",
                          headers={"Accept":"application/json, text/javascript, */*; q=0.01",
                                   "X-Requested-With":"XMLHttpRequest",
                                   "Referer": f"{BASE_URL}/portal/numbers/test"})
        h = decode_response(r) if r else ""
        results["test2_datatable_xhr"] = {
            "status": r.status_code if r else None,
            "content_type": r.headers.get("content-type","") if r else "",
            "preview": h[:800]
        }
    except Exception as e:
        results["test2_datatable_xhr"] = {"error": str(e)}

    # Test 3: GET /portal/numbers (my numbers)
    try:
        r, _ = do_request(account, "GET", f"{BASE_URL}/portal/numbers?draw=1&start=0&length=5&search[value]=&search[regex]=false",
                          headers={"Accept":"application/json, text/javascript, */*; q=0.01",
                                   "X-Requested-With":"XMLHttpRequest",
                                   "Referer": f"{BASE_URL}/portal/numbers"})
        h = decode_response(r) if r else ""
        results["test3_my_numbers"] = {
            "status": r.status_code if r else None,
            "content_type": r.headers.get("content-type","") if r else "",
            "preview": h[:800]
        }
    except Exception as e:
        results["test3_my_numbers"] = {"error": str(e)}

    return jsonify({"account": email, "results": results})
