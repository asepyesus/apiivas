...
# ════════════════════════════════════════════════════════
# NUMBER MANAGEMENT — /portal/numbers & /portal/numbers/test
# Endpoints discovered from JS DataTables:
#   GET  data    : /portal/numbers/test (DataTables serverSide)
#   POST add     : /portal/numbers/termination/number/add
#   POST delete  : /portal/numbers/termination/details  (modal TerminationDetials)
#   GET  my nums : /portal/numbers  (My Numbers page, also DataTables)
# ════════════════════════════════════════════════════════
...
def numbers_test_list():
    ...
    Ambil daftar Test Numbers via DataTables AJAX.
    Query params: range (filter by range name), limit (default 100)

    iVAS endpoint: GET /portal/numbers/test?draw=1&start=0&length=N&search[value]=...
    Columns: range, test_number, term, A2P(rate), Limit_Range, limit_did_a2p, limit_cli_did_a2p
    """
    sessions = login_all_accounts()
    if not sessions:
        return jsonify({"error": "Semua akun gagal login"}), 500
    ...
    for session in target_sessions:
        ...
        try:
            ...
            url = f"{BASE_URL}/portal/numbers/test?{qs}"

            resp, _ = do_request(account, "GET", url,
            ...
            # Response bisa JSON (DataTables) atau HTML
            try:
                data = resp.json()
                rows = data.get("data", [])
                for row in rows:
                    ...
                    if isinstance(row, list):
                        entry.update({
                            "range_name":  re.sub(r"<[^>]+>", "", str(row[0])).strip() if len(row) > 0 else "",
                            "test_number": re.sub(r"<[^>]+>", "", str(row[1])).strip() if len(row) > 1 else "",
                            "term":        re.sub(r"<[^>]+>", "", str(row[2])).strip() if len(row) > 2 else "",
                            "rate":        re.sub(r"<[^>]+>", "", str(row[3])).strip() if len(row) > 3 else "",
                            "limit_range": re.sub(r"<[^>]+>", "", str(row[4])).strip() if len(row) > 4 else "",
                            "sid_did":     re.sub(r"<[^>]+>", "", str(row[5])).strip() if len(row) > 5 else "",
                        })
                        # Cari number_id dari kolom Action (terakhir)
                        action_html = str(row[-1]) if row else ""
                        m = re.search(r"TerminationDetials\s*\(\s*(\d+)\s*\)", action_html)
                        if m:
                            entry["number_id"] = m.group(1)
                    elif isinstance(row, dict):
                        entry.update({
                            "range_name":  str(row.get("range", row.get("range_name", ""))),
                            "test_number": str(row.get("test_number", row.get("number", ""))),
                            "term":        str(row.get("term", "")),
                            "rate":        str(row.get("A2P", row.get("rate", ""))),
                            "limit_range": str(row.get("Limit_Range", "")),
                        })
                    all_numbers.append(entry)

                logger.info(f"[TEST-LIST] {email}: {len(rows)} nomor")

            except Exception:
                # Fallback: parse HTML
                html_r = decode_response(resp)
                soup_r = BeautifulSoup(html_r, "html.parser")
                for tbl in soup_r.find_all("table"):
                    ths = [th.get_text(strip=True).lower() for th in tbl.find_all("th")]
                    if "range" not in " ".join(ths):
                        continue
                    for row in tbl.find_all("tr"):
                        tds = row.find_all("td")
                        if len(tds) < 2:
                        ...
                        entry = {
                            "account":     email,
                            "range_name":  tds[0].get_text(strip=True),
                            "test_number": tds[1].get_text(strip=True) if len(tds) > 1 else "",
                            "term":        tds[2].get_text(strip=True) if len(tds) > 2 else "",
                            "rate":        tds[3].get_text(strip=True) if len(tds) > 3 else "",
                        }
                        all_numbers.append(entry)
...
def add_number():
    """
    POST /numbers/add
    Tambah nomor ke range di Test Numbers.

    Body JSON atau query params:
      - range_name : nama range persis seperti di tabel, contoh "PERU 543"
      - number     : nomor telepon, contoh "51910550499"
      - account    : (opsional) email akun

    iVAS endpoint: POST /portal/numbers/termination/number/add
    """
    ...
    if not target_sessions:
        return jsonify({"error": f"Akun '{account_email}' tidak ditemukan"}), 404
    ...
    try:
        # Step 1: GET halaman test → dapat csrf terbaru
        resp_page, csrf = do_request(
            account, "GET",
        ...
        # Step 2: POST ke /portal/numbers/termination/number/add
        ...
        try:
            ...
            status  = jr.get("status", jr.get("success", ""))
            success = str(status).lower() in ("success", "ok", "true", "1") or status is True or status == 1
        except Exception:
            if any(k in html_add.lower() for k in ("success", "berhasil", "added", "number added")):
                success = True
                message = "Number added successfully"
            elif any(k in html_add.lower() for k in ("error", "gagal", "failed", "already exist")):
                success = False
                message = "Failed — number may already exist or range not found"
            else:
                success = resp_add.status_code in (200, 201)
                message = f"HTTP {resp_add.status_code}"
...
def delete_number():
    """
    POST /numbers/delete
    Hapus/return nomor ke sistem.

    Body JSON atau query params:
      - number_id : ID nomor dari TerminationDetials(id) — dapat dari /numbers/test-list
      - number    : (alternatif) nomor telepon, akan dicari ID-nya dulu
      - account   : (opsional) email akun

    iVAS endpoint: POST /portal/numbers/termination/details
    """
    if request.method == "GET":
    ...
    if not number_id and not number:
        return jsonify({
            ...
            "contoh": "/numbers/delete?number_id=12345  ATAU  /numbers/delete?number=51910550499"
        }), 400
    ...
    # Kalau hanya punya number, cari number_id dulu via DataTables
    if not number_id and number:
        for session in target_sessions:
            email_s  = session["email"]
            account_s = next((a for a in ACCOUNTS if a["email"] == email_s), None)
            if not account_s:
                continue
            try:
                qs   = f"draw=1&start=0&length=500&search[value]={number}&search[regex]=false"
                resp_search, _ = do_request(
                    account_s, "GET",
                    f"{BASE_URL}/portal/numbers/test?{qs}",
                    headers=ajax_hdrs(referer=f"{BASE_URL}/portal/numbers/test"),
                )
                if resp_search:
                    try:
                        jr = resp_search.json()
                        for row in jr.get("data", []):
                            action_html = str(row[-1]) if isinstance(row, list) and row else ""
                            m = re.search(r"TerminationDetials\s*\(\s*(\d+)\s*\)", action_html)
                            if m:
                                number_id = m.group(1)
                                account_email = email_s
                                break
                    except Exception:
                        pass
            except Exception:
    ...
    if not number_id:
        return jsonify({
            "status":  "error",
            "error":   f"number_id tidak ditemukan untuk number={number}. Gunakan /numbers/test-list untuk lihat number_id.",
        }), 404

    # Eksekusi delete
    if account_email:
        target_sessions = [s for s in sessions if s["email"] == account_email] or target_sessions
    ...
    for session in target_sessions:
        ...
        if not account:
            continue
        try:
            del_url  = f"{BASE_URL}/portal/numbers/termination/details"
            resp_del, _ = do_request(
                account, "POST", del_url,
                data={"id": number_id},
                headers=ajax_hdrs(referer=f"{BASE_URL}/portal/numbers/test"),
            )
            if resp_del is None:
            ...
            try:
                ...
                if any(k in html_del.lower() for k in ("success", "returned", "berhasil", "deleted")):
                    success = True
                    message = "Number returned/deleted successfully"
                else:
                    success = resp_del.status_code in (200, 201)
                    message = f"HTTP {resp_del.status_code}"
            ...
            return jsonify({
                ...
                "message":      message,
                "del_url":      del_url,
                "raw_response": html_del[:500],
            })

        except Exception as e:
            logger.error(f"[DELETE-NUMBER] Error {email}: {e}")

    return jsonify({"status": "error", "error": "Semua akun gagal eksekusi delete"}), 500
...
def numbers_my_list():
    """
    GET /numbers/my-list
    Ambil daftar My Numbers via DataTables AJAX (/portal/numbers).
    Query params: limit (default 100), account (filter akun)
    """
    sessions = login_all_accounts()
    if not sessions:
        return jsonify({"error": "Semua akun gagal login"}), 500
    ...
    for session in target_sessions:
        ...
        if not account:
            continue
        try:
            qs  = f"draw=1&start=0&length={limit}&search[value]={search}&search[regex]=false"
            url = f"{BASE_URL}/portal/numbers?{qs}"
            ...
            try:
                ...
                for row in rows:
                    if isinstance(row, list):
                        action_html = str(row[-1]) if row else ""
                        # Cari number_id dari TerminationDetials(id)
                        m_id = re.search(r"TerminationDetials\s*\(\s*(\d+)\s*\)", action_html)
                        all_numbers.append({
                            "account":     email,
                            "number":      re.sub(r"<[^>]+>", "", str(row[0])).strip() if len(row) > 0 else "",
                            "range_name":  re.sub(r"<[^>]+>", "", str(row[1])).strip() if len(row) > 1 else "",
                            "rates":       re.sub(r"<[^>]+>", "", str(row[2])).strip() if len(row) > 2 else "",
                            "limit_range": re.sub(r"<[^>]+>", "", str(row[3])).strip() if len(row) > 3 else "",
                            "number_id":   m_id.group(1) if m_id else "",
                        })
                    elif isinstance(row, dict):
                        all_numbers.append({
                            "account":    email,
                            "number":     str(row.get("number", row.get("test_number", ""))),
                            "range_name": str(row.get("range_name", row.get("range", ""))),
                            "rates":      str(row.get("rates", row.get("rate", ""))),
                            "number_id":  str(row.get("id", "")),
                        })
                logger.info(f"[MY-LIST] {email}: {len(rows)} nomor")
                ...
                for tbl in soup_r.find_all("table"):
                    for row in tbl.find_all("tr"):
                        ...
                        all_numbers.append({
                            ...
                            "range_name": tds[1].get_text(strip=True) if len(tds) > 1 else "",
                        })
...
def debug_numbers_page():
    """Debug: raw info dari halaman /portal/numbers atau /portal/numbers/test."""
    page          = request.args.get("page", "my")
    account_email = request.args.get("account", "")
    ...
    try:
        ...
        ajax_urls  = list(dict.fromkeys(ajax_urls))

        script_snippets = []
        for script in soup.find_all("script"):
            txt = script.get_text()
            if any(k in txt for k in ("numbers", "store", "return", "termination", "DataTable", "TerminationDetials")):
                snippet = txt.strip()[:2000]
                if snippet:
