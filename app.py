...
def debug_numbers_page():
    ...
    try:
        ...
        for i, tbl in enumerate(soup.find_all("table")):
            ...
            rows = []
            for row in tbl.find_all("tr")[:5]:  # 5 baris pertama saja
                tds = row.find_all("td")
                if tds:
                    rows.append({
                        "cells": [td.get_text(strip=True)[:80] for td in tds],
                        "onclicks": [td.get("onclick", "") for td in tds if td.get("onclick")],
                        "buttons": [
                            {"tag": str(btn)[:200]}
                            for btn in row.find_all(["button", "a"])
                        ],
                    })
            tables_info.append({"index": i, "headers": ths, "sample_rows": rows})
