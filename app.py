...
# NUMBER MANAGEMENT
# Dari raw-debug:
#   Data endpoint : GET /portal/numbers/test (DataTables, dengan header XHR)
#   Response dict : {id, range, test_number, A2P, term, Limit_Range,
#                    limit_did_a2p, limit_cli_did_a2p, created_at, action}
#   Add number    : POST /portal/numbers/termination/number/add
#   Delete/return : POST /portal/numbers/termination/details  {id: number_id}
#   My Numbers    : GET /portal/numbers (DataTables sama)
# ════════════════════════════════════════════════════════

def _fetch_datatables(account, url, search="", length=100, extra_headers=None):
    """
    Helper: fetch DataTables JSON dari iVAS.
    Return (list_of_rows, recordsTotal) atau ([], 0) kalau gagal.
    rows adalah list of dict.
    """
    qs = (
        f"draw=1"
        f"&columns[0][data]=range&columns[0][name]=terminations.range"
        f"&columns[1][data]=test_number&columns[1][name]=terminations.test_number"
    ...
    resp, _ = do_request(account, "GET", f"{url.split('?')[0]}?{qs}", headers=hdrs)
    if resp is None or resp.status_code != 200:
        return [], 0
    try:
        data = resp.json()
        ...
        # Pastikan rows adalah list of dict
        if rows and isinstance(rows[0], list):
            # Kalau ternyata list-of-list, konversi pakai field names
            fields = ["range", "test_number", "term", "A2P", "Limit_Range",
                      "limit_did_a2p", "limit_cli_did_a2p", "created_at", "action"]
...
def _extract_number_id(row):
    """Ambil number_id dari row dict — bisa dari field 'id' atau dari HTML 'action'."""
    if row.get("id"):
        return str(row["id"])
    # Parse dari HTML action: onclick="TerminationDetials('788486')"
    action = str(row.get("action", "") or row.get("DT_RowId", ""))
    m = re.search(r"TerminationDetials\s*\(\s*['\"]?(\d+)['\"]?\s*\)", action)
    if m:
        return m.group(1)
    m2 = re.search(r"DT_RowId.*?(\d+)", action)
    if m2:
        return m2.group(1)
    return ""
...
def numbers_test_list():
    ...
    Query params:
      search  : filter (range name / nomor)
      limit   : jumlah data, default 100 (max 685984)
      account : filter akun tertentu
    """
    sessions = login_all_accounts()
    if not sessions:
    ...
    target = [s for s in sessions if s["email"] == account_email] if account_email else sessions
    ...
    for session in target:
        ...
        try:
            ...
            total_records = max(total_records, total)
            for row in rows:
                nid = _extract_number_id(row)
                all_numbers.append({
                    "account":           email,
                    "number_id":         nid,
                    "range_name":        str(row.get("range", "")),
                    "test_number":       str(row.get("test_number", "")),
            ...
            logger.info(f"[TEST-LIST] {email}: {len(rows)}/{total} nomor")
        except Exception as e:
...
def numbers_my_list():
    ...
    Sama strukturnya dengan test-list.
    """
    sessions = login_all_accounts()
    if not sessions:
        return jsonify({"error": "Login gagal"}), 500
        ...
        try:
            ...
            for row in rows:
                ...
                all_numbers.append({
                    "account":     email,
                    "number_id":   nid,
                    "number":      str(row.get("test_number", row.get("number", ""))),
                    "range_name":  str(row.get("range", row.get("range_name", ""))),
                    "rate_a2p":    str(row.get("A2P", row.get("rate", ""))),
                    "limit_range": str(row.get("Limit_Range", "")),
                    "created_at":  str(row.get("created_at", "")),
                })
            logger.info(f"[MY-LIST] {email}: {len(rows)}/{total}")
        except Exception as e:
            errors.append({"account": email, "error": str(e)})
...
def add_number():
    ...
    Params (JSON body, form, atau query):
      range_name : nama range persis, contoh "PERU 543"
    ...
    if request.method == "GET":
        ...
        range_name    = d.get("range_name","") or request.form.get("range_name","")
        number        = d.get("number","")     or request.form.get("number","")
        account_email = d.get("account","")    or request.form.get("account","")
    ...
    target  = [s for s in sessions if s["email"] == account_email] if account_email else sessions[:1]
    if not target:
    ...
    try:
        ...
        resp, _ = do_request(
            ...
            data={"number": number, "range_name": range_name},
            headers={
                "Accept":           "application/json, text/javascript, */*; q=0.01",
                "X-Requested-With": "XMLHttpRequest",
        ...
        try:
            ...
            if any(k in raw.lower() for k in ("success","berhasil","added")):
                success, message = True, "Number added successfully"
            else:
...
def delete_number():
    ...
    Params:
      number_id : ID nomor dari test-list (field number_id)
                  ATAU
      number    : nomor telepon → auto search number_id dulu
      account   : (opsional) email akun

    iVAS endpoint: POST /portal/numbers/termination/details  {id: number_id}
    """
    if request.method == "GET":
        ...
        d             = request.get_json(silent=True) or {}
        number_id     = d.get("number_id","")  or request.form.get("number_id","")
        number        = d.get("number","")      or request.form.get("number","")
        account_email = d.get("account","")     or request.form.get("account","")
    ...
    if not number_id and not number:
        return jsonify({
            ...
            "contoh_1": "/numbers/delete?number_id=788486",
            "contoh_2": "/numbers/delete?number=93766304206",
        }), 400
    ...
    # Kalau hanya punya number → cari number_id dulu
    found_range = ""
    if not number_id and number:
        for session in target:
            ...
            rows, _ = _fetch_datatables(
                account_s, f"{BASE_URL}/portal/numbers/test",
                search=number, length=200
            )
            for row in rows:
                num_clean = re.sub(r"\D", "", str(row.get("test_number", "")))
                if num_clean == re.sub(r"\D", "", number):
                    number_id     = _extract_number_id(row)
                    found_range   = str(row.get("range", ""))
    ...
    if not number_id:
        return jsonify({
            "status": "error",
            "error":  f"number_id tidak ditemukan untuk number={number}. Cek /numbers/test-list.",
        }), 404

    # Re-filter target kalau account_email sudah diketahui
    if account_email:
        target = [s for s in sessions if s["email"] == account_email] or target
    ...
    for session in target:
        ...
        try:
            ...
            try:
                ...
                if any(k in raw.lower() for k in ("success","returned","berhasil","deleted")):
                    success, message = True, "Number returned successfully"
                else:
                    success = resp.status_code in (200, 201)
                    message = f"HTTP {resp.status_code}"
            ...
            logger.error(f"[DELETE] Error {email}: {e}")
...
def debug_numbers_page():
    """Debug: raw info halaman /portal/numbers atau /portal/numbers/test."""
    ...
    try:
        ...
        ajax_urls = list(dict.fromkeys(
            re.findall(r"""ajax\s*[=:]\s*['"]([^'"]+)['"]""", html) +
            re.findall(r"""url\s*:\s*['"]([^'"]+portal[^'"]+)['"]""", html)
        ))
        script_snippets = [
            s.get_text().strip()[:1500]
            for s in soup.find_all("script")
            if any(k in s.get_text() for k in ("numbers","termination","DataTable","TerminationDetials"))
        ][:4]

        return jsonify({
            "status": "ok", "account": email,
            "page": page, "url": url,
            "http_status": resp.status_code,
            "html_length": len(html),
